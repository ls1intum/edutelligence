...
def transcribe_with_azure_whisper(audio_path: str, llm_id="azure-whisper") -> dict:
    ...
    chunk_paths = split_audio_ffmpeg(
        audio_path, chunks_dir, chunk_duration=180
    )  # 3-minute chunks
    ...
    for i, chunk_path in enumerate(chunk_paths):
        ...
        for attempt in range(max_retries):
            with open(chunk_path, "rb") as f:
                logger.info(
                    f"📤 Uploading chunk {i+1}/{len(chunk_paths)}: {chunk_path}"
                )
                ...
                try:
                    ...
                    if response.status_code == 429:
                        wait = 10 * attempt
                        logger.warning(
                            f"429 Too Many Requests. Retrying in {wait}s (Attempt {attempt+1}/{max_retries})..."
                        )
                        time.sleep(wait)
                        continue
                    ...
                    logger.error(f"❌ Azure Whisper HTTP error: {response.text}")
                except Exception as e:
                    logger.error(f"❌ Failed to process chunk {chunk_path}: {e}")
                # File is reopened in next iteration if needed

        if not success:
            raise RuntimeError(f"⚠️ Too many retries for chunk {chunk_path}")
