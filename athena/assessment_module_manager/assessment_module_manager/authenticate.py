...
def verify_lms_athena_key(lms_url: str, secret: str):
    ...
    print(f"Verifying LMS Athena key for URL: {lms_url} with secret: {secret}")
    print(f"Deployment secrets: {env.DEPLOYMENT_SECRETS}")
    if (
        lms_url not in env.DEPLOYMENT_SECRETS
        or secret != env.DEPLOYMENT_SECRETS[lms_url]
    ):
        if env.PRODUCTION:
