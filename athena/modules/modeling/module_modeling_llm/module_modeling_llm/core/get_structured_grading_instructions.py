...
from athena.storage.structured_grading_criterion_storage import (
    get_structured_grading_criterion,
    store_structured_grading_criterion,
)
...
from module_modeling_llm.prompts.structured_grading_instructions_prompt import (
    StructuredGradingInstructionsInputs,
)
...
async def get_structured_grading_instructions(
    ...
    cached_instructions = get_structured_grading_criterion(
        exercise_model.exercise_id, current_hash
    )
    ...
    chat_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                config.generate_suggestions_prompt.structured_grading_instructions_system_message,
            ),
            (
                "human",
                config.generate_suggestions_prompt.structured_grading_instructions_human_message,
            ),
        ]
    )
    ...
    if debug:
        emit_meta(
            "get_structured_grading_instructions",
            {
                ...
                "result": (
                    grading_instruction_result.dict()
                    if grading_instruction_result is not None
                    else None
                ),
            },
        )
    ...
    if not grading_instruction_result:
        raise ValueError(
            "No structured grading instructions were returned by the model."
        )

    # Cache the grading instructions
    hash = get_grading_instructions_hash(exercise_model)
    store_structured_grading_criterion(
        exercise_model.exercise_id, hash, grading_instruction_result
    )
