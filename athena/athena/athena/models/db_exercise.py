...
class DBExercise(Model, Base):
    ...
    id = Column(
        BigIntegerWithAutoincrement, primary_key=True, index=True, nullable=False
    )
    ...
    structured_grading_criterion = relationship(
        "DBStructuredGradingCriterion", back_populates="exercise", uselist=False
    )

    # Polymorphism, discriminator attribute
    type = Column(SqlEnum(ExerciseType), index=True, nullable=False)

    __mapper_args__ = {"polymorphic_identity": "exercise", "polymorphic_on": "type"}
