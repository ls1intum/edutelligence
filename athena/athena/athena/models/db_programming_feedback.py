...
class DBProgrammingFeedback(DBFeedback, Base):
    ...
    exercise_id = Column(
        ...
        ForeignKey("programming_exercises.id", ondelete="CASCADE"),
        index=True,
    )
    submission_id = Column(
        BigIntegerWithAutoincrement,
        ForeignKey("programming_submissions.id", ondelete="CASCADE"),
        index=True,
    )
    ...
    def get_referenced_code(self) -> str:
        ...
        with get_db() as db:
            db_submission = (
                db.query(DBProgrammingSubmission).filter_by(id=self.submission_id).one()
            )
            code = cast(ProgrammingSubmission, db_submission.to_schema()).get_code(
                self.file_path
            )
            ...
            if self.line_end is None:
                raise ValueError(
                    "Unexpected: line_start is not None, but line_end is None (only line_start given)"
                )
