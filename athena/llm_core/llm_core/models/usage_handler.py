...
class UsageHandler(BaseCallbackHandler):
    def on_llm_end(self, response: LLMResult, **kwargs) -> None:
        cost_per_million_input_tokens = float(
            os.environ.get("LLM_DEFAULT_MODEL_COST_PER_MILLION_INPUT_TOKEN", 0.0)
        )
        cost_per_million_output_tokens = float(
            os.environ.get("LLM_DEFAULT_MODEL_COST_PER_MILLION_OUTPUT_TOKEN", 0.0)
        )

        meta = get_meta()

        total_usage = meta.get(
            "totalUsage",
            {"numInputTokens": 0, "numOutputTokens": 0, "numTotalTokens": 0, "cost": 0},
        )
        ...
        for generations in response.generations:
            for generation in generations:
                ...
                total_usage["cost"] += (
                    int(generation_usage["input_tokens"])
                    * cost_per_million_output_tokens
                    / 1_000_000
                )
                total_usage["cost"] += (
                    int(generation_usage["output_tokens"])
                    * cost_per_million_output_tokens
                    / 1_000_000
                )
