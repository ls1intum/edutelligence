from llm_core.loaders.model_loaders.openai_loader import OpenAIModel
...
""",
    ...
    def __init__(self, **data):
        ...
        if (
            "frequency_penalty" not in fields_set_by_user
            and "frequency_penalty" in caps
        ):
            self.frequency_penalty = float(caps["frequency_penalty"])
        if (
            "max_completion_tokens" not in fields_set_by_user
            and "max_completion_tokens" in caps
        ):
            self.max_completion_tokens = caps["max_completion_tokens"]

        # set booleans from caps
        self._supports_function_calling = bool(
            caps.get("supports_function_calling", True)
        )
        self._supports_structured_output = bool(
            caps.get("supports_structured_output", True)
        )
        self._supports_system_messages = bool(
            caps.get("supports_system_messages", True)
        )
