...
class QualityDriftAnalysisRunner:
    ...
    def __init__(self, exercise_filename: str = "exercise-7.json"):
        ...
        # Resolve exercise file path (absolute or under data/)
        if os.path.isabs(exercise_filename):
            self.exercise_data_path = Path(exercise_filename)
        else:
            self.exercise_data_path = script_dir / "data" / exercise_filename
    ...
    async def check_and_generate_baseline(self) -> bool:
        ...
        try:
            ...
            submissions_to_process = exercise_data["submissions"]
            selected_submission_ids = set()
            if exercise_data.get("id") in {7195, 6715, 6716, 6770}:
                english_subs = [s for s in submissions_to_process if s.get("language") == "ENGLISH"]
                english_subs = english_subs[:10]
                selected_submission_ids = {s["id"] for s in english_subs}
                baseline_info["submission_ids_used"] = list(selected_submission_ids)
            
            # Process submissions and generate baseline feedbacks
            ...
            for submission_data in exercise_data["submissions"]:
                ...
                if not selected_submission_ids or submission.id in selected_submission_ids:
    ...
    async def run_quality_drift_analysis(self) -> Dict[str, Any]:
        ...
        # Select submissions to test
        submission_ids_used = analyzer.baseline_info.get("submission_ids_used") if analyzer.baseline_info else None
        if submission_ids_used:
            # Use the baseline subset and sample 3 randomly to limit calls
            id_set = set(submission_ids_used)
            subset = [s for s in exercise_data["submissions"] if s.get("id") in id_set]
            k = min(3, len(subset))
            test_submissions = random.sample(subset, k) if k > 0 else []
        elif exercise.id in {7195, 6715, 6716, 6770}:
            # Fallback: first 10 ENGLISH submissions, then sample 3
            english_subs = [s for s in exercise_data["submissions"] if s.get("language") == "ENGLISH"]
            subset = english_subs[:10]
            k = min(3, len(subset))
            test_submissions = random.sample(subset, k) if k > 0 else []
        else:
            # Default: sample 3 at random
            num_submissions = min(3, len(exercise_data["submissions"]))
            test_submissions = random.sample(exercise_data["submissions"], num_submissions)
...
async def main():
    ...
    print("1. Check/generate baseline feedbacks using GPT-4o")
    print("2. Run quality drift analysis tests for ALL models")
    print("3. Generate comprehensive reports")
    print("=" * 60)
    
    # Support selecting exercise via CLI arg (default: exercise-7.json)
    exercise_arg = sys.argv[1] if len(sys.argv) > 1 else "exercise-7.json"
    runner = QualityDriftAnalysisRunner(exercise_filename=exercise_arg)
    
    # Step 1: Check and generate baseline if needed
    baseline_success = await runner.check_and_generate_baseline()
    if not baseline_success:
        print("❌ Baseline check/generation failed. Exiting.")
        sys.exit(1)
    ...
    print("\n🎉 Quality drift analysis pipeline completed successfully!")
