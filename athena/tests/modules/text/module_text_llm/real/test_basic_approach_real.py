...
async def test_generate_suggestions_algorithm_explanation(real_config):
    ...
    found_complexity = [
        term for term in required_complexity_terms if term in all_feedback
    ]
    min_required = len(required_complexity_terms) // 2
    assert (
        len(found_complexity) >= min_required
    ), f"Feedback must include at least {min_required} complexity terms. Found: {', '.join(found_complexity)}"

    required_algorithm_terms = ["sorted", "interval", "element"]
    found_algorithm = [
        term for term in required_algorithm_terms if term in all_feedback
    ]
    min_required = len(required_algorithm_terms) // 2
    assert (
        len(found_algorithm) >= min_required
    ), f"Feedback must include at least {min_required} algorithm terms. Found: {', '.join(found_algorithm)}"
...
async def test_generate_suggestions_code_documentation(real_config):
    ...
    assert (
        len(found_doc) >= min_required
    ), f"Feedback must include at least {min_required} documentation terms. Found: {', '.join(found_doc)}"
...
async def test_generate_suggestions_design_pattern(real_config):
    ...
    assert (
        len(found_pattern) >= min_required
    ), f"Feedback must include at least {min_required} design pattern terms. Found: {', '.join(found_pattern)}"
