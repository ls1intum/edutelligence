#!/usr/bin/env python3
"""
Quality Drift Analysis Runner for Modeling Module

This script:
1. Generates baseline feedbacks using GPT-4o for modeling exercises (only if needed)
...
class QualityDriftAnalyzer:
    ...
    def load_baseline_feedbacks(self, exercise_data_path: str):
        """Load baseline feedbacks from exercise JSON file."""
    ...
    async def generate_test_feedbacks(self, config: BasicApproachConfig, exercise_model: ExerciseModel, structured_grading_instructions: StructuredGradingCriterion):
        """Generate feedbacks using the test model."""
        feedbacks = await generate_suggestions(
            exercise_model=exercise_model,
            structured_grading_instructions=structured_grading_instructions,
            config=config,
            debug=False,
        )
        ...
        for feedback in feedbacks.feedbacks:
            test_feedbacks.append({
                ...
                "title": feedback.title,
                "credits": feedback.credits,
                "element_name": feedback.element_name,
                "grading_instruction_id": feedback.grading_instruction_id,
            })
...
class QualityDriftAnalysisRunner:
    """Runs comprehensive quality drift analysis for modeling exercises."""
    
    def __init__(self, exercise_filename: str = "exercise-15949.json"):
    ...
    async def check_and_generate_baseline(self) -> bool:
        ...
        try:
            ...
            # Create ExerciseModel object
            exercise_model = ExerciseModel(
                exercise_id=exercise_data["id"],
                submission_id=0,  # Will be set per submission
                ...
                transformed_example_solution=exercise_data.get("example_solution", ""),
                submission_uml_type="ClassDiagram",  # Default UML type
                transformed_submission="",  # Will be set per submission
                element_id_mapping={},  # Empty mapping for baseline generation
                id_type_mapping={},  # Empty mapping for baseline generation
            )
            ...
            for submission_data in exercise_data["submissions"]:
                # Update exercise model for this submission
                exercise_model.submission_id = submission_data["id"]
                exercise_model.transformed_submission = submission_data.get("model", "")
                
                print(f"Generating baseline feedbacks for submission {exercise_model.submission_id}...")
                
                # Get structured grading instructions for this submission
                structured_grading_instructions = None
                if "structured_grading_instructions" in submission_data:
                    # Use the first set of structured grading instructions
                    sgi_data = submission_data["structured_grading_instructions"][0]
                    structured_grading_instructions = StructuredGradingCriterion(**sgi_data)
                else:
                    # Create a default one if none exists
                    structured_grading_instructions = StructuredGradingCriterion(
                        id=1,
                        title="Default Grading Criterion",
                        description="Default grading criterion for modeling exercise",
                        max_points=exercise_data["max_points"],
                        criteria=[]
                    )
                ...
                baseline_feedbacks = []
                for i, feedback in enumerate(feedbacks.feedbacks):
                    baseline_feedbacks.append({
    ...
    async def run_quality_drift_analysis(self) -> Dict[str, Any]:
        ...
        for model_info in model_configs:
            ...
            for submission_data in test_submissions:
                # Create ExerciseModel for this submission
                exercise_model = ExerciseModel(
                    exercise_id=exercise_data["id"],
                    ...
                    transformed_example_solution=exercise_data.get("example_solution", ""),
                    submission_uml_type="ClassDiagram",  # Default UML type
                    transformed_submission=submission_data.get("model", ""),
                    element_id_mapping={},  # Empty mapping for test generation
                    id_type_mapping={},  # Empty mapping for test generation
                )
                ...
                print(f"  Generating feedbacks for submission {submission_data['id']}...")
                
                # Generate test feedbacks
                test_feedbacks = await analyzer.generate_test_feedbacks(
                    config, exercise_model, structured_grading_instructions
                )
                
                # Store test feedbacks
                analyzer.test_feedbacks[submission_data['id']] = test_feedbacks
                
                # Analyze quality drift
                result = analyzer.analyze_quality_drift(submission_data['id'])
...
async def main():
    ...
    print("🎯 Quality Drift Analysis Pipeline for Modeling Module")
    print("=" * 60)
