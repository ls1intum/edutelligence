...
async def test_sgi_ecommerce_system(real_config):
    ...
    submission = Submission(id=10, exerciseId=exercise.id, model=ECOMMERCE_SUBMISSION)
    ...
    assert (
        credits_by_id[2] == 0.0
    ), "Order-Product should be association, not composition"
...
async def test_sgi_hospital_management(real_config):
    ...
    submission = Submission(id=1, exerciseId=exercise.id, model=HOSPITAL_SUBMISSION)
    ...
    assert (
        credits_by_id[1] == 0.0
    ), "Appointment class is missing date and status attributes"
    assert credits_by_id[2] >= 1.0, "Doctor must inherit from Person"
    assert (
        credits_by_id[4] == 0.0
    ), "Patient has patientId and medicalHistory attributes"
