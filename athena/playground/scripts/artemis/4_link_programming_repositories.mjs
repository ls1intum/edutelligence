...
for (let exercise of exercises) {
  ...
  if (!fs.existsSync(path.join(exercisePath, "solution"))) {
    ...
    exercise.solution_repository_uri = `{{exerciseDataUrl}}/solution.json`;
  }
  ...
  if (!fs.existsSync(path.join(exercisePath, "template"))) {
    ...
    exercise.template_repository_uri = `{{exerciseDataUrl}}/template.json`;
  }
  ...
  if (!fs.existsSync(path.join(exercisePath, "tests"))) {
    ...
    exercise.tests_repository_uri = `{{exerciseDataUrl}}/tests.json`;
  }
  ...
  exercise.submissions = exercise.submissions.map((submission) => {
    ...
    if (!fs.existsSync(submissionPath)) {
      ...
      submission.repository_uri = `{{exerciseDataUrl}}/submissions/${submission.id}.json`;
    }
    return submission;
  });
