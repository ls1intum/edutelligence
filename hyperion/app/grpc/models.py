...
class RepositoryFile(GrpcMessage):
    ...
    def to_grpc(self) -> hyperion_pb2.RepositoryFile:
        return hyperion_pb2.RepositoryFile(
            path=self.path,
...
class Repository(GrpcMessage):
    ...
    files: List[RepositoryFile] = Field(default_factory=list, description="Files in the repository")
    
    def to_grpc(self) -> hyperion_pb2.Repository:
        return hyperion_pb2.Repository(
            files=[file.to_grpc() for file in self.files]
        )
    
    @classmethod
    def from_grpc(cls, grpc_repo: hyperion_pb2.Repository) -> "Repository":
        return cls(
            files=[RepositoryFile.from_grpc(file) for file in grpc_repo.files]
        )
...
class BoundaryConditions(GrpcMessage):
    ...
    programming_language: int = Field(..., description="Programming language enum value")
    ...
    def to_grpc(self) -> hyperion_pb2.SolutionRepositoryCreatorRequest:
        ...
        raise NotImplementedError("BoundaryConditions should be part of a request/response")
...
class ProblemStatement(GrpcMessage):
    ...
    def to_grpc(self) -> hyperion_pb2.SolutionRepositoryCreatorRequest:
        # Since we flattened the structure, we need to create a partial request
        # This method should not be called directly
        raise NotImplementedError("ProblemStatement should be part of a request/response")
