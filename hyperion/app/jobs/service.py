...
from app.actions.base_models import ActionUpdate, JobStatus
...
async def send_callback(
    job: Job,
    status: JobStatus,
    update: Optional[ActionUpdate] = None,
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(
                f"Callback sent successfully for job {job.job_id} to {job.callback_url}, status: {status}"
            )
        except httpx.RequestError as e:
            logger.error(
                f"Error sending callback for job {job.job_id} to {job.callback_url}: {e}"
            )
        except httpx.HTTPStatusError as e:
            logger.error(
                f"Callback for job {job.job_id} to {job.callback_url} failed with "
...
async def process_job(job_id: str):
    ...
    async def send_action_update(update: ActionUpdate):
        ...
        await send_callback(job, job.status, update=update)
        logger.info(
            f"Sending update for job {job.job_id} with action name '{job.action_name}' "
            f"status '{job.status}' and type '{update.update_type}' to {job.callback_url}."
        )
    ...
    try:
        ...
        logger.exception(
            f"Error processing job {job.job_id} with action name '{job.action_name}' : {e}"
        )
        job.status = JobStatus.FAILED
        job.error_message = str(e)
        job.updated_at = datetime.now(timezone.utc).isoformat()
...
class JobService(Generic[ActionInputT, ActionUpdateT]):
    ...
    async def create_job(
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"AI action '{job_request.action_name}' not registered.",
            )
        ...
        if callback_auth_secret:
            auth_details = CallbackAuth(
                header_name=callback_auth_header_name, secret=callback_auth_secret
            )

        job = Job[ActionInputT, ActionUpdateT](
            job_id=job_id,
        ...
        jobs_db[job_id] = job
        logger.info(
            f"Created job {job_id} for action '{job.action_name}'. Callback to: {job.callback_url}"
        )
