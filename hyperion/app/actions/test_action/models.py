...
class TestActionInput(ActionInput):
    ...
    options: Optional[dict] = Field(None, description="Additional options for processing")
...
class TestActionProgressUpdate(ProgressUpdate):
...
class TestActionResult(ResultUpdate):
