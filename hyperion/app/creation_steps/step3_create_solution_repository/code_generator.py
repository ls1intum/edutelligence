...
class CodeGenerator:
    ...
    def _parse_solution_plan_response(self, response: str) -> SolutionPlan:
        ...
        try:
            ...
            architecture_description: str = parsed_data.get("architecture_description", "")
            ...
            if len(architecture_description) == 0:
                print("No architecture description found in response, using raw response")
        ...
        print("Using fallback solution plan with raw response as architecture description")
        return SolutionPlan(
            architecture_description=response,
            required_classes=[],
            required_functions=[],
    ...
    async def _step_1_2_define_file_structure(self, context: SolutionCreationContext) -> SolutionCreationContext:
        ...
        response = await self.model.ainvoke(prompt)
        file_structure: FileStructure = self._parse_file_structure_response(response.content)
    ...
    def _build_file_structure_prompt(self, context: SolutionCreationContext) -> str:
        ...
        architecture_desc: str = solution_plan.architecture_description if solution_plan else "No solution plan available"
        required_classes: List[str] = solution_plan.required_classes if solution_plan else []
        required_functions: List[str] = solution_plan.required_functions if solution_plan else []
    ...
    def _parse_file_structure_response(self, response: str) -> FileStructure:
        ...
        try:
            ...
            directories: List[str] = [d.strip().replace('\\', '/') for d in directories if d.strip()]
            files: List[str] = [f.strip().replace('\\', '/') for f in files if f.strip()]
            build_files: List[str] = [b.strip().replace('\\', '/') for b in build_files if b.strip()]
            
            print(f"Successfully parsed file structure: {len(directories)} dirs, {len(files)} files, {len(build_files)} build files")
    ...
    async def _step_1_3_generate_headers(self, context: SolutionCreationContext) -> SolutionCreationContext:
        ...
        for file_path in source_files:
            ...
            response = await self.model.ainvoke(prompt)
            headers_content: str = self._parse_file_headers_response(response.content, file_path)
    ...
    def _build_file_headers_prompt(self, context: SolutionCreationContext, file_path: str) -> str:
        ...
        all_files: List[str] = context.file_structure.files if context.file_structure else []
    ...
    def _parse_file_headers_response(self, response: str, file_path: str) -> str:
        ...
        try:
            ...
            print(f"Successfully parsed headers for {file_path}: {len(content)} characters")
            return content
    ...
    def _determine_file_purpose(self, file_path: str, context: SolutionCreationContext) -> str:
        ...
        elif file_name.endswith('util.py') or file_name.endswith('utils.py') or file_name.endswith('helper.py'):
    ...
    async def _step_1_4_generate_core_logic(self, context: SolutionCreationContext) -> SolutionCreationContext:
        ...
        for file_path in source_files:
            ...
            prompt: str = self._build_implementation_prompt(context, file_path, existing_content)
            response = await self.model.ainvoke(prompt)
    ...
    def _build_implementation_prompt(self, context: SolutionCreationContext, file_path: str, existing_content: str) -> str:
        ...
        required_functions: List[str] = solution_plan.required_functions if solution_plan else []
        algorithms: List[str] = solution_plan.algorithms if solution_plan else []
        design_patterns: List[str] = solution_plan.design_patterns if solution_plan else []
    ...
    def _get_other_files_context(self, context: SolutionCreationContext, current_file_path: str) -> str:
        ...
        other_files = [f for f in context.file_structure.files if f != current_file_path]
        ...
        for file_path in other_files[:5]:
            try:
                ...
                print(f"   [CodeGenerator] Could not read file {file_path} for context: {e}")
    ...
    def _get_file_preview(self, content: str, file_path: str) -> str:
        ...
        for i, line in enumerate(lines[:15]):
            ...
            if (stripped.startswith(('import ', 'from ', 'class ', 'def ', 'public ', 'private ', 'protected ')) or
    ...
    def _create_solution_repository(self, context: SolutionCreationContext) -> SolutionCreationContext:
        ...
        repo_files = []
        all_files = (context.file_structure.files or []) + (context.file_structure.build_files or [])
        for file_path in all_files:
            try:
                ...
                logger.error(f"Could not read file '{file_path}' to create repository object: {e}")
