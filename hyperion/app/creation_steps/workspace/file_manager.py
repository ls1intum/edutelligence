...
from ..step3_create_solution_repository.models import (
    FileStructure,
...
class FileManager:
    ...
    def create_file_structure(
        self, context: SolutionCreationContext, file_structure: FileStructure
    ) -> None:
    ...
    def copy_file(
        ...
        if not source_path or not dest_path:
            raise FileSystemException(
                "Source and destination paths cannot be empty", file_path=source_path
            )
        ...
        try:
            ...
            if not source_full.exists():
                raise FileSystemException(
                    f"Source file does not exist: {source_path}", file_path=source_path
                )

            if not source_full.is_file():
                raise FileSystemException(
                    f"Source path is not a file: {source_path}", file_path=source_path
                )
    ...
    def delete_file(self, context: SolutionCreationContext, file_path: str) -> None:
        ...
        try:
            ...
            if full_path.is_file():
                ...
                raise FileSystemException(
                    f"Path is a directory, not a file: {file_path}", file_path=file_path
                )
            else:
                raise FileSystemException(
                    f"Path is neither file nor directory: {file_path}",
                    file_path=file_path,
                )
    ...
    def create_directory(self, context: SolutionCreationContext, dir_path: str) -> None:
        ...
        if not dir_path:
            raise FileSystemException(
                "Directory path cannot be empty", file_path=dir_path
            )
    ...
    def list_files(
        ...
        try:
            ...
            if not full_path.exists():
                raise FileSystemException(
                    f"Directory does not exist: {dir_path}", file_path=dir_path
                )

            if not full_path.is_dir():
                raise FileSystemException(
                    f"Path is not a directory: {dir_path}", file_path=dir_path
                )
            ...
            items.sort()
            logger.debug(
                f"Listed {len(items)} items in directory: {dir_path or 'root'}"
            )
            return items
    ...
    def set_file_permissions(
        ...
        try:
            ...
            if (
                ...
                raise FileSystemException(
                    f"Invalid permissions value: {permissions}", file_path=file_path
                )
    ...
    def _resolve_workspace_path(
        ...
        try:
            if not context.workspace_path:
                raise FileSystemException(
                    "Workspace path is not set in context", file_path=relative_path
                )
    ...
    def _validate_path_in_workspace(
        ...
        try:
            if not context.workspace_path:
                raise FileSystemException(
                    "Workspace path is not set in context", file_path=str(path)
                )
            ...
            for component in suspicious_components:
                if component in path_str:
                    logger.warning(
                        f"Suspicious path component detected: {component} in {path}"
                    )
