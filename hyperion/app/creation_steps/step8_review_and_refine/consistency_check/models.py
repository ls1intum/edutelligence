...
from app.grpc.hyperion_pb2 import ArtifactLocation, ArtifactType, ConsistencyIssue, ConsistencyIssueCategory, ConsistencyIssueSeverity, ConsistencyIssueType
...
class StructuralConsistencyIssueCategory(str, Enum):
  METHOD_SIGNATURE_MISMATCH = "METHOD_SIGNATURE_MISMATCH";                 # Method return type, parameters, or visibility differs between artifacts
  CONSTRUCTOR_SIGNATURE_MISMATCH = "CONSTRUCTOR_SIGNATURE_MISMATCH";       # Constructor parameters differ between specification and template
  INTERFACE_IMPLEMENTATION_CONFLICT = "INTERFACE_IMPLEMENTATION_CONFLICT"; # Required interface cannot be implemented as specified
  TYPE_DECLARATION_CONFLICT = "TYPE_DECLARATION_CONFLICT";                 # Data types inconsistent across artifacts
  INHERITANCE_HIERARCHY_MISMATCH = "INHERITANCE_HIERARCHY_MISMATCH";       # Extends/implements relationships differ between UML and template
  PACKAGE_STRUCTURE_MISMATCH = "PACKAGE_STRUCTURE_MISMATCH";               # Import/package organization prevents compilation
  MISSING_REQUIRED_ELEMENT = "MISSING_REQUIRED_ELEMENT";                   # Essential class/method/attribute missing from template
...
class ConsistencyIssueModel(BaseModel):
    ...
    primary_location: ArtifactLocationModel = Field(description="Primary location where issue was detected")
    related_locations: List[ArtifactLocationModel] = Field(description="Related locations across artifacts")
    suggested_fix: str = Field(description="Actionable correction to resolve the mismatch")
...
class StructuralConsistencyIssueModel(ConsistencyIssueModel):
    ...
    description: str = Field(description="Clear explanation of the signature mismatch")
    severity: SeverityEnum = Field(description="Student impact severity level")
    category: StructuralConsistencyIssueCategory = Field(description="Specific category of structural consistency issue")
    primary_location: ArtifactLocationModel = Field(description="Primary location where issue was detected")
    related_locations: List[ArtifactLocationModel] = Field(description="Related locations across artifacts")
    suggested_fix: str = Field(description="Actionable correction to resolve the mismatch")
...
class ConsistencyResult(BaseModel):
    ...
    issues: List[ConsistencyIssueModel] = Field(description="List of detected consistency issues")

class StructuralConsistencyResult(ConsistencyResult):
    """Result of structural consistency issue analysis."""
    issues: List[StructuralConsistencyIssueModel] = Field(description="List of detected structural consistency issues")


# Without category similar as in TypeScript with Omit
def convert_consistency_issue_to_protobuf(issue: ConsistencyIssueModel) -> ConsistencyIssue:
    ...
    for rel_loc in issue.related_locations:
        related_locations.append(ArtifactLocation(
    ...
    type_map = {
        "STRUCTURAL": ConsistencyIssueType.STRUCTURAL,
        "SEMANTIC": ConsistencyIssueType.SEMANTIC,
        "ASSESSMENT": ConsistencyIssueType.ASSESSMENT,
        "PEDAGOGICAL": ConsistencyIssueType.PEDAGOGICAL,
    }
    ...
    category_key = str(issue.category) if isinstance(issue.category, int) else issue.category
