...
class ModelPerformancePlotter:
    ...
    def _load_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("JSON data must be a dictionary of model names to lists of runs.")
            return data
        ...
        for model_name, runs in self.data.items():
            ...
            if valid_runs:
                ...
                summary[model_name] = {
                    ...
                    'correlation': pearsonr(tokens, f1_scores)[0] if len(tokens) > 1 else None,
                    'p_value': pearsonr(tokens, f1_scores)[1] if len(tokens) > 1 else None
                }
    ...
    def print_correlation_analysis(self):
        ...
        print("=" * 60)
        print(f"{'Model Name':<25} | {'Correlation':<11} | {'P-Value':<8} | {'N Samples':<9}")
        print("-" * 60)
        
        for model_name, runs in self.data.items():
            valid_runs = self._filter_valid_runs(runs)  # X=tokens, Y=F1            
            if len(valid_runs)>1: #for model correlation we need at least 2 data points
                tokens = [run['prompt_tokens'] for run in valid_runs]
                f1_scores = [run['f1'] for run in valid_runs]
                try:
                    corr, p_value = pearsonr(tokens, f1_scores)
                    if np.isnan(corr) or np.isnan(p_value):
                        print(f"{model_name:<25} | {'Invalid':<11} | {'N/A':<8} | {len(tokens):<9}")
                    else:
                        significance = ""
                        if p_value < 0.001:
                            significance = "***"
                        ...
                        print(f"{model_name:<25} | {corr:8.3f}{significance:<3} | {p_value:8.3f} | {len(tokens):<9}")
                except Exception as e:
                    print(f"{model_name:<25} | {'Error':<11} | {'N/A':<8} | {len(tokens):<9}"+f" (Error: {e})")
            else:
                print(f"{model_name:<25} | {'N/A':<11} | {'N/A':<8} | {len(valid_runs):<9}")
    ...
    def print_per_exercise_correlation_analysis(self):
        ...
        print("=" * 80)
        print(f"{'Model':<25} | {'Exercise':<20} | {'Correlation':<11} | {'P-Value':<8} | {'N':<5}")
        print("-" * 80)
        ...
        for model_name, runs in self.data.items():
            ...
            if not valid_runs:
                print(f"{model_name:<25} | {'No valid data':<20} | {'N/A':<11} | {'N/A':<8} | {'0':<5}")
                continue
            ...
            for exercise, data in model_data_all_ex.items():
                ...
                exercise_short_name = exercise.split('-')[0] if '-' in exercise else exercise[:20]
                
                if len(tokens_exercise) > 1:
                    try:
                        corr, p_value = pearsonr(tokens_exercise, f1_exercise)
                        if np.isnan(corr) or np.isnan(p_value):
                            print(f"{model_name:<25} | {exercise_short_name:<20} | {'Invalid':<11} | {'N/A':<8} | {len(tokens_exercise):<5}")
                        else:
                            significance = ""
                            if p_value < 0.001:
                                significance = "***"
                                ...
                                significance = "*"
                        print(f"{model_name:<25} | {exercise_short_name:<20} | {corr:8.3f}{significance:<3} | {p_value:8.3f} | {len(tokens_exercise):<5}")
                    except Exception as e:
                        print(f"{model_name:<25} | {exercise_short_name:<20} | {'Error':<11} | {'N/A':<8} | {len(tokens_exercise):<5}"+f" (Error: {e})")
                else:
                    print(f"{model_name:<25} | {exercise_short_name:<20} | {'N/A':<11} | {'N/A':<8} | {len(tokens_exercise):<5}")
    ...
    def plot_per_model_subplots(self, save_path: str = None, color_by_exercise: bool = True):
        """
        ...
        all_exercises = set() # all possible exercises in the data, model to model can derive
        ...
        for i, exercise in enumerate(sorted(all_exercises)):
            exercise_colors[exercise] = colors[i % len(colors)]  # Cycle through colors if many exercises
        ...
        for i, (model_name, runs) in enumerate(self.data.items()):
            ...
            if not valid_runs:
                axe.text(0.5, 0.5, 'No valid data', ha='center', va='center',
                    transform=axe.transAxes, fontsize=12)  # Use axes coordinates (0-1)
            ...
            if color_by_exercise and exercise_colors:
                ...
                for exercise, data in model_data_all_ex.items():
                    ...
                    if ex_tokens:
                        ...
                        label = exercise.split('-')[0] if '-' in exercise else exercise[:10]
                        axe.scatter(ex_tokens, ex_f1, c=color, alpha=0.7, s=50, # scatter plot (alpha = 0.8?)
                                label=label, edgecolors='white', linewidth=0.5)
                        
                # Add points color explanation if space allows
                if len(all_exercises) <= 5:
                    axe.legend(bbox_to_anchor=(1.05, 1), loc='upper left', frameon=False)  # outside plot area                    
            else:
                # Plot all points in same color
                axe.scatter(tokens, f1_scores, c='#1f77b4', alpha=0.7, s=50,
                    edgecolors='white', linewidth=0.5)            
            
            # OLS Trend Line
            if len(tokens) > 1:                              
                try:
                    z = np.polyfit(tokens, f1_scores, 1)     # linear regression e.g [-0.0002, 1.1] - slope and intercept
                    p = np.poly1d(z)                         # polynomial function  y=-0.0002x + 1.1, p is a function from x (token), which returns y (F1)
                    x_trend = np.linspace(min(tokens), max(tokens), 100) # generates 100 points between min and max tokens
    ...
    def plot_per_model_per_exercise_subplots(self, save_path: str = None):
        ...
        fig, axes = plt.subplots(n_models, n_exercises,
                                figsize=(4*n_exercises, 3*n_models)) # 4 inches width per exercise, 3 inches height per model
        ...
        for i, (model_name, runs) in enumerate(self.data.items()):
            ...
            if not valid_runs:
                for j, exercise in enumerate(exercise_list):
                    ...
                    axe.text(0.5, 0.5, 'No valid data', ha='center', va='center',
                        transform=axe.transAxes, fontsize=12)
                    exercise_short_name = exercise.split('-')[0] if '-' in exercise else exercise[:10]
            ...
            for j, exercise in enumerate(exercise_list):
                ...
                if exercise in model_data_all_ex:
                    ...
                    if ex_tokens:
                        color = exercise_colors.get(exercise, '#999999')
                        axe.scatter(ex_tokens, ex_f1, c=color, alpha=0.7, s=50,
                                edgecolors='white', linewidth=0.5)
                        ...
                        if len(ex_tokens) > 1:
                            try:
                                ...
                                p = np.poly1d(z)
                                x_trend = np.linspace(min(ex_tokens), max(ex_tokens), 100)
                                axe.plot(x_trend, p(x_trend), "r--", alpha=0.8, linewidth=2)
                            except np.RankWarning:
                                pass
                        ...
                        title = f'{model_name}\n{exercise_short_name}\nNo data'
                        axe.text(0.5, 0.5, 'No data', ha='center', va='center',
