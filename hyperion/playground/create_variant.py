...
class VariantManager:
    ...
    def get_next_variant_number(self) -> str:
        ...
        existing_variants = [d.name for d in variants_dir.iterdir() if d.is_dir() and d.name.isdigit()]
    ...
    def init_variant(self, category: str, description: str, variant_id: str = None) -> str:
        ...
        print(f"🚀 Initializing variant {variant_id} for exercise: {self.exercise_path}")
        ...
        print(f"💡 Next: Make your changes, then run finalize")
        return variant_id
    ...
    def create_git_diff(self, variant_id: str) -> Path:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir_str:
                ...
                items_to_copy = ["problem-statement.md", "solution", "template", "tests"]
                ...
                try:
                    ...
                    if result.stdout:
                        ...
                        print(f"  ✓ Generated git diff with {len(diff_content.splitlines())} lines")
                        return patch_file
    ...
    def finalize_variant(self, variant_id: str) -> Path:
        ...
        print(f"💡 Next: python playground/create_variant.py --exercise \"{self.exercise_path}\" generate-annotation -v {variant_id}")
        return patch_file
    ...
    def generate_annotation(self, variant_id: str) -> Path:
        ...
        try:
            ...
            cmd = [sys.executable, str(eval_script), "--exercise", variant_exercise_path]
            
            result = subprocess.run(cmd, cwd=Path(__file__).parent.parent, capture_output=True, text=True)
            
            if result.returncode == 0:
                print(f"  ✓ Consistency checker completed")
                ...
                if output_file.exists():
                    ...
                    print(f"  ✓ Generated annotation with {len(clean_annotation['issues'])} detected issues")
                    
                    # Show what was detected
                    for issue in clean_annotation["issues"]:
                        print(f"    - {issue['category']} ({issue['severity']}): {issue['description'][:80]}...")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Unified CLI tool for stepwise consistency check variant creation")
    
    # Common arguments
    parser.add_argument("--exercise", "-e", required=True, help="Exercise path (e.g., ITP2425/H01E01-Lectures)")
    ...
    init_parser.add_argument("--category", "-c", required=True, choices=list(ALL_CATEGORIES.keys()), help="Consistency issue category")
    init_parser.add_argument("--description", "-d", required=True, help="Description of the consistency issue to inject")
    ...
    generate_parser = subparsers.add_parser("generate-annotation", help="Run consistency checker and create annotation")
    ...
    try:
        ...
        if args.command == "init":
            ...
            print(f"🔄 Then run: python playground/create_variant.py --exercise \"{args.exercise}\" finalize -v {variant_id}")
