...
export const SearchParamEnumOrUndefined = <T extends EnumLike>(searchParams: URLSearchParams, key: string, enumType: T) => {
    const value = searchParams.get(key);
    if (!value) {
        return undefined;
    }
...
export const ErrorResponse = (error: unknown, status : number) => NextResponse.json({ error: error?.toString() || "An unknown error occurred" }, { status: status })
