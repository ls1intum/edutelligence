...
export async function PUT(request: NextRequest) {
    ...
    if (!parsed.success) {
        return NextResponse.json({message: `Could not register endpoint activity - Invalid body - ${parsed.error}`}, {status: 400});
    }
    ...
    return NextResponse.json({message: `Successfully registered endpoint activity`}, {status: 200});
}
...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            const metricsService = applicationContainer.getMetricsService();
            const endpointActivity = await metricsService.getEndpointActivity(service, version, endpoint, type, from, to);
