...
def mock_generate_embeddings_openai():
    with patch("atlasml.ml.embeddings.generate_embeddings_openai", return_value=[0.1, 0.2, 0.3]), \
...
class MockWeaviateClient:
    ...
    def get_all_embeddings(self, collection_name: str):
        ...
        for obj in collection.iterator():
            ...
            results.append({
                "id": obj.uuid,
                "vector": {"default": obj.vector},  # Wrap vector in dict with "default" key
    ...
    def update_property_by_id(self, collection_name: str, obj_id: str, properties: dict, vector=None):
...
def mock_weaviate_client():
    ...
    with patch('weaviate.connect_to_local', return_value=mock_client):
        ...
        with patch('atlasml.clients.weaviate.WeaviateClientSingleton._instance', None):
            ...
            with patch('atlasml.ml.pipeline_workflows.get_weaviate_client', return_value=mock_client):
                yield mock_client
...
def weaviate_test_data(mock_weaviate_client):
    ...
    competency_collection = mock_weaviate_client.collections.get(CollectionNames.COMPETENCY.value)
    ...
    exercise_collection = mock_weaviate_client.collections.get(CollectionNames.EXERCISE.value)
