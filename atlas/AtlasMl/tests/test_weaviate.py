...
from atlasml.clients.weaviate import WeaviateClient, CollectionNames, get_weaviate_client
...
def mock_weaviate():
    ...
    with patch('atlasml.clients.weaviate.weaviate.connect_to_local', return_value=mock_client), \
...
def test_get_embeddings_by_property(mock_weaviate):
    ...
    results = client.get_embeddings_by_property(collection_name, property_name, property_value)
...
def test_error_handling(mock_weaviate):
    ...
    mock_weaviate.collections.exists.return_value = False
    with pytest.raises(ValueError, match="Collection 'invalid_collection' does not exist"):
