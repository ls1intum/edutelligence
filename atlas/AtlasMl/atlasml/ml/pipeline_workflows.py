...
class PipelineWorkflows:
    ...
    def map_new_competency_to_exercise(self, exercise_id: int, competency_id: int):
        exercise_data = self.weaviate_client.get_embeddings_by_property(
            CollectionNames.EXERCISE.value, "exercise_id", exercise_id
        )
        competency_data = self.weaviate_client.get_embeddings_by_property(
            CollectionNames.COMPETENCY.value, "competency_id", competency_id
        )
    ...
    def map_competency_to_competency(
        self, source_competency_id: int, target_competency_id: int
    ):
        ...
        source_related_raw = source_competency_data[0]["properties"].get(
            "related_competencies", []
        )
        target_related_raw = target_competency_data[0]["properties"].get(
            "related_competencies", []
        )
        ...
        source_properties = {
            "competency_id": int(
                source_competency_data[0]["properties"]["competency_id"]
            ),
        ...
        if "cluster_id" in source_competency_data[0]["properties"]:
            source_properties["cluster_id"] = source_competency_data[0]["properties"][
                "cluster_id"
            ]
        if "cluster_similarity_score" in source_competency_data[0]["properties"]:
            source_properties["cluster_similarity_score"] = source_competency_data[0][
                "properties"
            ]["cluster_similarity_score"]
        self.weaviate_client.update_property_by_id(
            CollectionNames.COMPETENCY.value,
            source_competency_data[0]["id"],
            source_properties,
        )
        ...
        target_properties = {
            "competency_id": int(
                target_competency_data[0]["properties"]["competency_id"]
            ),
        ...
        if "cluster_id" in target_competency_data[0]["properties"]:
            target_properties["cluster_id"] = target_competency_data[0]["properties"][
                "cluster_id"
            ]
        if "cluster_similarity_score" in target_competency_data[0]["properties"]:
            target_properties["cluster_similarity_score"] = target_competency_data[0][
                "properties"
            ]["cluster_similarity_score"]
        self.weaviate_client.update_property_by_id(
            CollectionNames.COMPETENCY.value,
            target_competency_data[0]["id"],
            target_properties,
        )

    def save_competency_to_weaviate(self, competency: Competency):
        embedding = generate_embeddings_openai(
            competency.description if competency.description else competency.title
        )
        properties = {
    ...
    def save_exercise_to_weaviate(self, exercise: ExerciseWithCompetencies):
        ...
        properties = {
            ...
            "description": exercise.description,
            "competency_ids": (
                [comp_id for comp_id in exercise.competencies]
    ...
    def save_competency(
        ...
        if operation_type == OperationType.DELETE:
            ...
            if existing_competency:
                ...
                embedings = generate_embeddings_openai(
                    competency.description
                    if competency.description
                    else competency.title
                )
                properties = {
                    "competency_id": competency.id,
                    "title": competency.title,
            ...
            self.recluster_with_new_competencies(
                competency=competency, course_id=competency.course_id
            )
    ...
    def save_competencies(
        ...
        self.recluster_with_new_competencies(
            competency=competencies[0], course_id=course_id
        )
    ...
    def recluster_with_new_competencies(self, competency: Competency, course_id: int):
        ...
        if len(competencies) > len(exercises):
            return
        ...
        labels, centroids = apply_kmeans(
            matrix=exercise_embeddings, n_clusters=len(competencies)
        )
        ...
        competency_embeddings = np.array(
            [comp["vector"]["default"] for comp in competencies]
        )
        ...
        competency_indices.sort(
            key=lambda i: np.max(similarity_matrix[i]), reverse=True
        )

        for comp_idx in competency_indices:
            # Find the best available cluster for this competency
            available_clusters = [
                i for i in range(len(centroids)) if i not in used_clusters
            ]
            if not available_clusters:
                raise ValueError("Not enough clusters for all competencies")

            best_cluster_idx = max(
                available_clusters, key=lambda i: similarity_matrix[comp_idx][i]
            )
    ...
    def new_text_suggestion(
        ...
        if not clusters:
            ...
            return self.suggest_competencies_if_no_clusters(
                exercise_description, course_id
            )
        else:
            # There are clusters which also mean that there are competencies
            cluster_centers = [
                SemanticCluster(
                    cluster_id=entry["properties"]["cluster_id"],
            ...
            similarities_with_indices: list[tuple[float, SemanticCluster]] = [
                (
                    compute_cosine_similarity(
                        exercise_embedding, cluster.vector_embedding
                    ),
                    cluster,
                )
                for cluster in cluster_centers
            ]
            similarities_with_indices.sort(
                key=lambda x: x[0], reverse=True
            )  # Sort by similarity score descending

            top_k = (
                top_k
                if top_k < len(similarities_with_indices)
                else len(similarities_with_indices)
            )
            topk_competencies = []
    ...
    def suggest_competencies_by_similarity(
        ...
        """
        top_competencies_with_similarity_scores = self.new_text_suggestion(
            exercise_description, course_id, top_k
        )
        top_competencies = [
            competency
            for competency, similarity_score in top_competencies_with_similarity_scores
        ]
    ...
    def instructor_feedback_on_new_text(
        ...
        if new_exercise.competencies == []:
            return
        self.update_cluster_for_competencies(
            new_exercise_embedding,
            new_exercise.competencies,
            is_removal=False,
    ...
    def instructor_feedback(
        ...
        # If there are no competencies, then there is nothing to do
        if not updated_exercise.competencies:
            return
        ...
        if removed_competencies:
            self.update_cluster_for_competencies(
                updated_exercise_embedding, removed_competencies, is_removal=True
            )
            logger.info(f"Removed competencies: {list(removed_competencies)}")

        if added_competencies:
            self.update_cluster_for_competencies(
                updated_exercise_embedding, added_competencies, is_removal=False
            )
    ...
    def suggest_competency_relations(
        self, course_id: int
    ) -> CompetencyRelationSuggestionResponse:
        """Suggest competency relations based on embedding similarity."""
        course_competencies = self.weaviate_client.get_embeddings_by_property(
            CollectionNames.COMPETENCY.value, "course_id", course_id
        )
        competencies: list[Competency] = [
            Competency(
                id=competency["properties"]["competency_id"],
                title=competency["properties"]["title"],
                description=competency["properties"]["description"],
        ...
        embeddings = np.array(
            [competency["vector"]["default"] for competency in course_competencies]
        )
        relationship_matrix = generate_competency_relationship(embeddings, descriptions)
        competencyRelationSuggestionResponse = CompetencyRelationSuggestionResponse(
            relations=[]
        )
        for i in range(len(relationship_matrix)):
            for j in range(len(relationship_matrix)):
                relation_type: Optional[RelationType] = None
                if relationship_matrix[i][j] == "NONE":
                    continue
                if relationship_matrix[i][j] == "MATCH":
                    relation_type = RelationType.MATCH
                if relationship_matrix[i][j] == "REQUIRE":
                    relation_type = RelationType.REQUIRES
                if relationship_matrix[i][j] == "EXTEND":
                    relation_type = RelationType.EXTEND
                relation: CompetencyRelation = CompetencyRelation(
                    tail_id=competencies[j].id,
                    head_id=competencies[i].id,
    ...
    def suggest_competencies_if_no_clusters(
        ...
        top_competencies = [
            ...
            for comp, similarity_score in similarities
