...
class Settings(BaseModel):
    ...
    def _get_default_settings(cls):
        ...
        logger.warning("Using default settings - ensure environment variables are set for production")
        
        default_api_keys = [APIKeyConfig(token="default-test-token")]
        default_weaviate = WeaviateSettings(host="localhost", port=8080, grpc_port=50051)
    ...
    def get_settings(cls, use_defaults: bool = False):
        ...
        if missing_vars:
            ...
            if "PYTEST_CURRENT_TEST" in os.environ or "TEST" in os.environ.get("ENV", ""):
...
def get_settings(use_defaults: bool = None) -> Settings:
    ...
    if _settings is None:
        ...
        if use_defaults is None:
            use_defaults = ("PYTEST_CURRENT_TEST" in os.environ or 
                          "TEST" in os.environ.get("ENV", "") or
