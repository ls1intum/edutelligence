...
.. figure:: ./process-model.png
    :align: center
    :alt: Process Model - Activity Diagram
...
.. figure:: ./feature-proposal-flow.png
    :align: center
...
.. note::
      Make sure to follow the `Atlas Code and Design Guidelines <https://docs.artemis.cit.tum.de/dev/guidelines.html>`_.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
* Make sure all steps in the `Checklist <https://github.com/ls1intum/Atlas/blob/develop/.github/PULL_REQUEST_TEMPLATE.md>`_ are completed.
