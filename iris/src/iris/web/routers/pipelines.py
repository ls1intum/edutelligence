...
def run_chat_pipeline_worker(
    ...
    try:
        ...
        variant = find_variant(ChatPipeline.get_variants(), variant_id)
...
def run_global_search_pipeline_worker(dto: GlobalSearchRequestDTO, request_id: str):
    ...
    try:
        ...
        if intent == SearchIntent.TRIGGER_AI:
            # RUNNING update = the "thinking" signal for the client loading state.
            # SKIP_AI resolves in ~200ms inside the pipeline (sources only), so no
            # intermediate update is needed there.
            callback.update()
    ...
    match feature:
        case "CHAT":
            return get_available_variants(
                safe_get_variants(ChatPipeline.get_variants),
                available_llms,
            )
