...
# Scenario expectations are evaluated against activity names emitted by the
# production callables, not their create_tool_* factory names. Keeping this
# allowlist narrow makes a typo (or an intentional production rename) fail in
# the free validation phase instead of wasting a paid qualification run.
_OBSERVABLE_PRODUCTION_TOOLS = frozenset(
    {
        ...
        "memiris_find_similar_memories",
        "memiris_search_for_memories",
        "repository_files",
    }
)


def _validate_expected_tool_names(scenario: Scenario) -> None:
    expected = scenario.expectations
    names = set(
        expected.required_tools
        + expected.optional_tools
        + expected.forbidden_tools
        + expected.tool_order
    )
    unknown = names - _OBSERVABLE_PRODUCTION_TOOLS
    if unknown:
        raise ScenarioContractError(
            f"{scenario.id}: expected tool names are not production activities: "
            f"{sorted(unknown)}"
        )


def _validate_solution_files(scenario: Scenario, qa_root: Path) -> None:
    artifact_root = (qa_root / "artifacts").resolve()
    for reference in scenario.expectations.solution_files:
        relative = Path(reference)
        if relative.is_absolute() or ".." in relative.parts:
            raise ScenarioContractError(
                f"{scenario.id}: solution file escapes the QA artifact root"
            )
        cursor = qa_root / "artifacts"
        for part in relative.parts:
            cursor /= part
            if cursor.is_symlink():
                raise ScenarioContractError(
                    f"{scenario.id}: solution file may not traverse symlinks"
                )
        resolved = (artifact_root / relative).resolve()
        if artifact_root not in resolved.parents or not resolved.is_file():
            raise ScenarioContractError(
                f"{scenario.id}: solution file is missing: {reference}"
            )
        try:
            content = resolved.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError) as error:
            raise ScenarioContractError(
                f"{scenario.id}: cannot read solution file {reference}: {error}"
            ) from error
        if not content.strip():
            raise ScenarioContractError(
                f"{scenario.id}: solution file is empty: {reference}"
            )
...
def validate_scenario_contract(
    ...
    *,
    ...
    _validate_expected_tool_names(scenario)
    _validate_solution_files(scenario, qa_root)
    ...
    if scenario.mode == "PROGRAMMING_EXERCISE_CHAT" and not provenance:
        raise ScenarioContractError(
