...
from iris.domain.communication.communication_tutor_suggestion_pipeline_execution_dto import (  # noqa: E402,E501
...
def test_course_fixture_executes_current_production_course_tools():
    dto, metadata = _chat("course-exercise-priorities-high")
    with ScenarioAdapters(metadata):
        state = _state(dto, metadata)
        exercises = providers.provide_exercise_list(state)()
        course = providers.provide_course_details(state)()

    assert course["course_name"] == "Algorithms and Data Structures"
    by_id = {item["id"]: item for item in exercises}
    assert by_id[9001]["submissions"][0]["score"] == 6.0
    assert by_id[9002]["submissions"] == []
