...
def test_provider_offers_the_tool_when_a_position_has_material():
    state = SimpleNamespace(current_view_content_blocks=["page 3: content"])
...
def test_provider_stays_silent_without_a_current_position():
    ...
    assert provide_current_view_content(SimpleNamespace()) is None
    assert (
        provide_current_view_content(SimpleNamespace(current_view_content_blocks=[]))
        is None
    )
