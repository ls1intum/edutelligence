...
class Substitution:
    """Map type variables to types and compose mappings in application order."""
    ...
    def apply_type(self, value: Monotype) -> Monotype:
        if isinstance(value, TVariable) and value.identifier in self.mapping:
            return self.apply_type(self.mapping[value.identifier])
    ...
    def compose(self, earlier: "Substitution") -> "Substitution":
        combined = {
            identifier: self.apply_type(value)
            for identifier, value in earlier.mapping.items()
        }
        combined.update(self.mapping)
