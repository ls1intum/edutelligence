    ...
    def from_env(cls) -> "CalibrationConfig":
        ...
        def _parse_time(raw: str, default: time) -> time:
            ...
            try:
                ...
                logger.warning(
                    "CalibrationConfig: invalid time %r — using default %s",
                    raw,
                    default,
                )
                return default
        ...
        return cls(
            window_start=_parse_time(
                os.getenv("LOGOS_CALIB_WINDOW_START", ""), time(2, 0)
            ),
...
class CalibrationOrchestrator:
    ...
    async def _tick(self) -> None:
        ...
        if target is None:
            logger.debug(
                "CalibrationOrchestrator: all models calibrated or no idle worker available"
            )
            return
        ...
        try:
            await self._registry.send_command(
                ...
                params={
                    ...
                    "sleep_level": self._config.sleep_level,
                },
                timeout_seconds=30,
            )
    ...
    async def _cancel_all_running(self) -> None:
        ...
        for provider_id in self._facade.provider_ids():
            ...
            if not status.get("active"):
                ...
                continue
            provider_name = self._facade.get_provider_name(provider_id) or str(
                provider_id
            )
            logger.info(
