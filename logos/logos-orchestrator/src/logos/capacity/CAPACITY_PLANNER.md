...
The operator "Drain" path (`drain_lane_manually`) is the busy-lane counterpart of the manual sleep: it cold-marks the lane (no new requests routed to it), waits the bounded `DRAIN_TIMEOUT_SECONDS` for the in-flight ones to finish, then sleeps the lane — or unloads it when the host-RAM headroom check fails or the lane's backend reports `sleep_state=unsupported`. A lane that does not drain in time is left exactly as found. The manual path clears its cold mark on every exit (unlike the executor's reclaim sleeps, which keep theirs until a wake clears them), because the manual wake endpoint dispatches the command directly and would otherwise strand a woken lane outside the rotation.
