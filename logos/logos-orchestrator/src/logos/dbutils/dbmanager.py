...
class DBManager:
    ...
    def add_full_model(
        ...
        return self.rebalance_added_model(
            pk, worse_accuracy, worse_quality, worse_latency, worse_cost
        )
    ...
    def rebalance_updated_model(
        self, updated_model_id: int, category: str, feedback: Union[str, int]
    ):
    ...
    def add_billing(
        ...
        billing_id = self.insert(
            "token_prices",
            {
                ...
                "price_per_k_token": round(type_cost),
                "model_id": model_id,
            },
        )
    ...
    def generalstats(self, logos_key: str):
        ...
        key_count = self.session.execute(
    ...
    def get_request_log_stats(
        ...
        end_dt = (
            isoparse(end_date).astimezone(datetime.timezone.utc) if end_date else now
        )
        start_dt = (
        ...
        last_ts = self.session.execute(
            text(
                f"SELECT MAX({ts_expr}) AS last_ts FROM log_entry WHERE {ts_expr} BETWEEN :start_ts AND :end_ts"
            ),
            params,
        ).scalar()
        ...
        totals = {
            ...
            "avgQueueSeconds": (
                float(totals_row["avg_queue_seconds"])
                if totals_row.get("avg_queue_seconds") is not None
                else None
            ),
            "avgRunSeconds": (
                float(totals_row["avg_run_seconds"])
                if totals_row.get("avg_run_seconds") is not None
                else None
            ),
        }
        ...
        status_counts = {
            row["status"].lower(): int(row["count"]) for row in status_rows
        }
        ...
        model_breakdown = [
            {
                ...
                "avgQueueSeconds": (
                    float(row["avg_queue_seconds"])
                    if row["avg_queue_seconds"] is not None
                    else None
                ),
                "avgRunSeconds": (
                    float(row["avg_run_seconds"])
                    if row["avg_run_seconds"] is not None
                    else None
                ),
        ...
        time_series = [
            {
                ...
                "avgVram": (
                    float(row["avg_vram"]) if row["avg_vram"] is not None else None
                ),
            }
        ...
        if queue_row:
            queue_depth = {
                "avgEnqueueDepth": (
                    float(queue_row["avg_enqueue"])
                    if queue_row.get("avg_enqueue") is not None
                    else None
                ),
                "avgScheduleDepth": (
                    float(queue_row["avg_schedule"])
                    if queue_row.get("avg_schedule") is not None
                    else None
                ),
                "p95EnqueueDepth": (
                    float(queue_row["p95_enqueue"])
                    if queue_row.get("p95_enqueue") is not None
                    else None
                ),
                "p95ScheduleDepth": (
                    float(queue_row["p95_schedule"])
                    if queue_row.get("p95_schedule") is not None
                    else None
                ),
            }
            ...
            results.append(
                {
                    ...
                    "status": (
                        row["result_status"] if row["result_status"] else "pending"
                    ),
                    "timestamp": (
                        row["enqueue_ts"].isoformat() if row["enqueue_ts"] else None
                    ),
                    "duration": (
                        float(row["run_seconds"])
                        if row["run_seconds"] is not None
                        else None
                    ),
                    ...
                    "scheduled_ts": (
                        row["scheduled_ts"].isoformat() if row["scheduled_ts"] else None
                    ),
                    "request_complete_ts": (
                        row["request_complete_ts"].isoformat()
                        if row["request_complete_ts"]
                        else None
                    ),
                    "queue_seconds": (
                        float(row["queue_seconds"])
                        if row["queue_seconds"] is not None
                        else None
                    ),
                    "total_seconds": (
    ...
    def get_request_logs(self, logos_key: str, request_ids: list[str]):
        ...
        api_key_row = self.session.execute(
            text(
                "SELECT id FROM api_keys WHERE key_value = :logos_key AND is_active = true"
            ),
            {"logos_key": logos_key},
        ).fetchone()
            ...
            results.append(
                {
                    ...
                    "ttft_ms": (
                        float(row["ttft_ms"]) if row["ttft_ms"] is not None else None
                    ),
                    "total_latency_ms": (
                    ...
                    "scheduler_total_ms": (
                        float(row["scheduler_total_ms"])
                        if row["scheduler_total_ms"] is not None
                        else None
                    ),
                    "cold_start": row["cold_start"],
                    "queue_depth_at_arrival": (
                        int(row["queue_depth_at_arrival"])
                        if row["queue_depth_at_arrival"] is not None
                        else None
                    ),
                    "utilization_at_arrival": (
                    ...
                    "queue_depth_at_schedule": (
                        int(row["queue_depth_at_schedule"])
                        if row["queue_depth_at_schedule"] is not None
                        else None
                    ),
                    "priority_when_scheduled": row["priority_when_scheduled"],
                    "load_duration_ms": (
                    ...
                    "available_vram_mb": (
                        int(row["available_vram_mb"])
                        if row["available_vram_mb"] is not None
                        else None
                    ),
                    "azure_rate_remaining_requests": (
        ...
        missing_request_ids = [
            request_id for request_id in normalized_ids if request_id not in found_ids
        ]
    ...
    def connect_api_key_provider(
        self, logos_key: str, api_key_id: int, provider_id: int
    ):
        """
        ...
        return {
            "result": f"Granted provider {provider_id} access to API key {api_key_id}."
        }, 200
    ...
    def connect_api_key_model(self, logos_key: str, api_key_id: int, model_id: int):
        ...
        result = self.session.execute(
            sql, {"api_key_id": int(api_key_id), "model_id": int(model_id)}
        ).fetchone()
        self.session.commit()
    ...
    def create_application_key(
        self, logos_key: str, team_id: int, key_name: str, environment: str = "-"
    ):
        if not self.check_authorization(logos_key):
            return {"error": "Database changes only allowed for root user."}, 500
        ...
        if existing:
            return {
                "error": f"This team already has an active application key for environment '{environment}'."
            }, 400
    ...
    def disconnect_model_provider(
        self, logos_key: str, model_id: int, provider_id: int
    ):
        if not self.check_authorization(logos_key):
            return {"error": "Database changes only allowed for root user."}, 500
        ...
        result = self.session.execute(
            sql, {"mid": int(model_id), "pid": int(provider_id)}
        ).fetchone()
    ...
    def get_request_enqueues_deltas(
        ...
        if after_enqueue_ts:
            try:
                ...
                return {
                    "error": f"Invalid after_enqueue_ts format: {after_enqueue_ts}"
                }, 400
    ...
    def add_model_provider_api_key(
        ...
        if c != 200:
            return r, c
        model_id = r["model_id"]
        r, c = self.connect_model_provider(
            logos_key, model_id, provider_id, api_key=api_key, endpoint=model_endpoint
        )
        if c != 200:
            return r, c
        ...
        if c != 200:
            return r, c
        return {
            "result": f"Successfully added model and connected to api_key_id {api_key_id}"
        }, 200

    def set_api_key_log(self, api_key_id: int, log_level: str):
        if log_level not in {"BILLING", "FULL"}:
            return {
                "error": "Invalid logging level. Choose between 'BILLING' and 'FULL'"
            }, 400
        ...
        self.session.execute(
            sql, {"log_level": log_level, "api_key_id": int(api_key_id)}
        )
        self.session.commit()
    ...
    def update_provider_info(
        self, logos_key: str, provider_id: int, **kwargs
    ) -> Tuple[dict, int]:
        if not self.check_authorization(logos_key):
        ...
        if (
            "privacy_level" in updates
            and updates["privacy_level"] not in VALID_PRIVACY_LEVELS
        ):
    ...
    def set_user_role(self, user_id: int, role: str):
        ...
        if role not in valid:
            return {
                "error": f"Invalid role '{role}'. Must be one of: {sorted(valid)}"
            }, 400
        sql = text(
            """
                   UPDATE users
    ...
    def create_user(
        ...
        for t_id in final_team_ids:
            t_row = self.session.execute(
                text("SELECT name FROM teams WHERE id = :tid"), {"tid": t_id}
            ).fetchone()
            if not t_row:
                continue
            ...
            self.insert(
                "team_members", {"user_id": user_id, "team_id": t_id, "is_owner": False}
            )
    ...
    def update_user_info(
        ...
        if email is not None and email.strip():
            existing = self.session.execute(
                text(
                    "SELECT id FROM users WHERE lower(email) = lower(:email) AND id != :user_id"
                ),
        ...
        sql_str = (
            f"UPDATE users SET {', '.join(updates)} WHERE id = :user_id RETURNING id"
        )
    ...
    def import_users_from_csv(self, file_bytes: bytes) -> dict:
        ...
        if missing:
            return {
                "error": f"Missing required CSV headers: {', '.join(sorted(missing))}"
            }
        ...
        for row in reader:
            ...
            if team_names:
                ...
                if missing_teams:
                    rows_out.append(
                        fail(f"Team(s) not found: {', '.join(missing_teams)}")
                    )
                    continue
            ...
            try:
                ...
                if existing:
                    ...
                    for tid in team_ids:
                        if not self._is_team_member(tid, user_id):
                            ...
                            key_row = self.session.execute(
                                text(
                                    "SELECT key_value FROM api_keys WHERE user_id = :uid AND team_id = :tid"
                                ),
                                {"uid": user_id, "tid": tid},
                            ).fetchone()
                    ...
                    if status != 200:
                        rows_out.append(
                            fail(user_dict.get("error", "User creation failed"))
                        )
                        continue
    ...
    def list_teams(
        self, user_id: int | None = None, is_logos_admin: bool = False
    ) -> list[dict]:
        if is_logos_admin:
            sql = text(
                """
    ...
    def create_team(
        ...
        for user_id in owner_ids:
            existing_member = self.session.execute(
                text(
                    "SELECT user_id FROM team_members WHERE team_id = :tid AND user_id = :uid"
                ),
    ...
    def delete_team(self, team_id: int):
        self.session.execute(
            text("DELETE FROM api_keys WHERE team_id = :tid"), {"tid": team_id}
        )

        self.session.execute(
            text("DELETE FROM teams WHERE id = :tid"), {"tid": team_id}
        )
        self.session.commit()
    ...
    def add_team_member(
        self, team_id: int, user_id: int, is_owner: bool = False
    ) -> tuple[dict, int]:
        row = self.session.execute(
            text(
                """
    ...
    def remove_team_member(self, team_id: int, user_id: int) -> tuple[dict, int]:
        ...
        if not result:
            row = self.session.execute(
                text(
                    "DELETE FROM team_members WHERE team_id = :tid AND user_id = :uid RETURNING user_id"
                ),
                {"tid": team_id, "uid": user_id},
            ).fetchone()
    ...
    def set_team_owner(
        self, team_id: int, user_id: int, is_owner: bool
    ) -> tuple[dict, int]:
        row = self.session.execute(
            text(
                """
    ...
    def get_api_key_model_permissions(self, api_key_id: int) -> list[int]:
        rows = self.session.execute(
            text(
                "SELECT model_id FROM api_key_model_permissions WHERE api_key_id = :aki"
            ),
            {"aki": api_key_id},
        ).fetchall()
        return [r.model_id for r in rows]
    ...
    def get_team_provider_permissions(self, team_id: int) -> list[int]:
        rows = self.session.execute(
            text(
                "SELECT provider_id FROM team_provider_permissions WHERE team_id = :tid"
            ),
            {"tid": team_id},
        ).fetchall()
    ...
    def get_api_key_provider_permissions(self, api_key_id: int) -> list[int]:
        rows = self.session.execute(
            text(
                "SELECT provider_id FROM api_key_provider_permissions WHERE api_key_id = :aki"
            ),
            {"aki": api_key_id},
        ).fetchall()
        return [r.provider_id for r in rows]

    def add_api_key_provider_permission(
        self, api_key_id: int, provider_id: int
    ) -> None:
        self.session.execute(
            text(
                """
    ...
    def get_key_budget_history(
        self, team_id: int, start: str, end: str, bucket_seconds: int
    ) -> list:
        interval = _bucket_to_pg_interval(bucket_seconds)
        rows = self.session.execute(
            text(
    ...
    def update_api_key(
        ...
        row = self.session.execute(
            text("SELECT settings FROM api_keys WHERE id = :id"), {"id": api_key_id}
        ).fetchone()
