...
class Balancer:
    ...
    def __init__(self):
        self.weights = {"policy": list(), "token": list(), "ai": list()}
    ...
    def get_weight(self):
        return (
            self.__combine_policy() * self.POLICY_WEIGHT
            + self.__combine_token() * self.TOKEN_WEIGHT
            + self.__combine_ai() * self.LAURA_WEIGHT
        )
