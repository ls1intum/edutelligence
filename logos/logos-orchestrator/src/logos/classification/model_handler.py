...
class ModelHandler:
    ...
    def __rebalance(models: List[Tuple[int, int]]):
        ...
        if len(models) & 0x1:
            ...
            center = (
                models[len(models) // 2 - 1][0] + models[len(models) // 2][0]
            ) // 2
    ...
    def add_model(self, worse_id: Union[int, None], model_id: int):
        ...
        reset = self.__rebalance(
            [(self.__score(x), j) for x, (_, j) in enumerate(self.models)]
        )
        # Obtain user feedback
        diffs = [
            reset[index][0] - self.models[index][0] for index in range(len(self.models))
        ]
        # Obtain feedback shift. This is necessary to prevent false differences after rebalancing
        feedback_shift = (
            (self.models[0][0] + self.models[-1][0]) // 2
    ...
    def give_feedback(self, model_id: int, feedback: int):
        ...
        while (
            ...
            and self.models[index][0] > self.models[index + 1][0]
        ):
            self.models[index], self.models[index + 1] = (
                self.models[index + 1],
                self.models[index],
    ...
    def __score(self, position: int):
        ...
        if self.unique == 0:
            return 0
        return self.feedback_scale * int(
            -self.base_feedback * self.unique + 2 * self.base_feedback * position
        )
