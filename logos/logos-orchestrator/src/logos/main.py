...
async def create_team_app_key(
    team_id: int, body: CreateAppKeyEndpointRequest, request: Request
):
    ...
    with DBManager() as db:
        ...
        if caller["role"] == "app_admin" and not db.is_team_owner(
            team_id, caller["id"]
        ):
...
async def delete_api_key(key_id: int, request: Request):
    ...
    with DBManager() as db:
        ...
        if caller["role"] == "app_admin":
            if not key_info.get("team_id") or not db.is_team_owner(
                key_info["team_id"], caller["id"]
            ):
                raise HTTPException(
                    status_code=403,
...
async def set_api_key_model_permissions(
    key_id: int, body: SetApiKeyModelPermissionsRequest, request: Request
):
    logos_key = require_app_admin_or_above(request)
    with DBManager() as db:
        key_info = db.get_api_key_by_id(key_id)
...
async def set_team_provider_perms(
    team_id: int, body: SetTeamProviderPermissionsRequest, request: Request
):
    with DBManager() as db:
        require_logos_admin(request)
...
async def set_api_key_provider_permissions(
    key_id: int, body: SetApiKeyProviderPermissionsRequest, request: Request
):
    logos_key = require_app_admin_or_above(request)
    with DBManager() as db:
        key_info = db.get_api_key_by_id(key_id)
...
async def create_user(body: CreateUserRequest, request: Request):
    ...
    with DBManager() as db:
        ...
        user_dict, user_keys, status = db.create_user(
            body.prename, body.name, body.email, body.role, body.team_ids
        )
    if status != 200:
...
async def patch_user_info(user_id: int, body: UpdateUserInfoRequest, request: Request):
    ...
    with DBManager() as db:
        ...
        if caller["role"] == "app_admin":
            if (
                target_user.role in ("app_admin", "logos_admin")
                and caller["id"] != user_id
            ):
                raise HTTPException(
                    status_code=403,
        ...
        result, status = db.update_user_info(
            user_id=user_id, prename=body.prename, name=body.name, email=body.email
        )
    ...
    with DBManager() as db:
        ...
        if caller["role"] == "app_admin":
            ...
            if caller["id"] == user_id:
                raise HTTPException(
                    status_code=403, detail="You cannot remove yourself from a team"
                )
...
async def set_team_model_perms(
    team_id: int, body: SetTeamModelPermissionsRequest, request: Request
):
    with DBManager() as db:
        require_logos_admin_or_team_owner(team_id, request, db)
...
async def set_team_member_owner(
    team_id: int, user_id: int, body: SetOwnerRequest, request: Request
):
    require_logos_admin(request)
    with DBManager() as db:
...
async def team_budget_history(data: BillingHistoryRequest, request: Request):
    ...
    with DBManager() as db:
        buckets = db.get_team_budget_history(
            data.start_iso, data.end_iso, bucket_seconds
        )
    return {
        "buckets": buckets,
...
async def key_budget_history(
    team_id: int, data: BillingHistoryRequest, request: Request
):
    import datetime as _dt
    ...
    with DBManager() as db:
        ...
        span_seconds = int((end - start).total_seconds())
        bucket_seconds = _choose_bucket_seconds(span_seconds)
        buckets = db.get_key_budget_history(
            team_id, data.start_iso, data.end_iso, bucket_seconds
        )
    return {
        "buckets": buckets,
        "bucket_seconds": bucket_seconds,
...
async def paginated_requests_endpoint(request: Request):
    ...
    with DBManager() as db:
        payload, status = db.get_paginated_requests(
            logos_key, page=page, per_page=per_page
        )
        return JSONResponse(content=payload, status_code=status)
...
def _build_vram_signature(providers: list) -> str:
    ...
    for p in sorted(providers, key=lambda x: x.get("name", "")):
        ...
        models_str = (
            ...
            if isinstance(last.get("loaded_models"), list)
            else ""
        )
        parts.append(
...
async def ws_stats_v2(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(
            "[ws/stats/v2] Client disconnected (%d remaining)",
            len(_ws_stats_v2_connections),
        )
