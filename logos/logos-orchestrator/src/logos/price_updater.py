...
async def _store_prices_for_pair(
    ...
    with DBManager() as db:
        for field, token_type in LITELLM_TO_TOKEN_TYPE.items():
            ...
            db.upsert_model_token_price(
                model_id, token_type, price_per_k, valid_from, provider_id=provider_id
            )
    logging.info(
...
async def fetch_and_store_prices() -> None:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            logging.info(
                "price_updater: catalog refreshed (%d models)", len(_catalog_cache)
            )
        except Exception as exc:
...
async def fetch_price_for_single_model(model_id: int, model_name: str) -> None:
    ...
    if not providers:
        logging.info(
            "price_updater: no cloud providers for '%s' (id=%s), skipping",
            model_name,
            model_id,
        )
        return
