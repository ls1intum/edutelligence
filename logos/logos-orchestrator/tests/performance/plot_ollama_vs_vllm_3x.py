#!/usr/bin/env python3
"""
Plot LogosWorkerNode (vLLM 3x oversubscription) vs Ollama total latency
distributions side-by-side.

For each workload size (150, 300, 600), overlays the overall total_latency_ms
distributions from both configurations on a single chart.
...
RUNS = [
    {
        "label": "150 requests",
        "tag": "150req",
        "vllm3x_csv": sorted((BASE / "results/oversubscribed_3x").glob("*150req_resolver60s_3x/detailed.csv"))[-1],
        "ollama_csv": sorted((BASE / "results_ollama/hw3_random_10m_150req").glob("*/results_detailed.csv"))[-1],
    },
    {
        "label": "300 requests",
        "tag": "300req",
        "vllm3x_csv": sorted((BASE / "results/oversubscribed_3x").glob("*300req_resolver60s_3x/detailed.csv"))[-1],
        "ollama_csv": sorted((BASE / "results_ollama/hw3_random_10m_300req").glob("*/results_detailed.csv"))[-1],
    },
    {
        "label": "600 requests",
        "tag": "600req",
        "vllm3x_csv": sorted((BASE / "results/oversubscribed_3x").glob("*600req_resolver60s_3x/detailed.csv"))[-1],
        "ollama_csv": sorted((BASE / "results_ollama/hw3_random_10m_600req").glob("*/results_detailed.csv"))[-1],
    },
]

OUT_DIR = BASE / "results_ollama_vs_vllm_3x"
...
def main() -> None:
    ...
    for run in RUNS:
        ...
        logos_data = load_latencies(logos_csv)
