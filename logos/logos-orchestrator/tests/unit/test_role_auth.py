...
import logos as role_auth  # triggers sys.modules["logos"] = logos.main

# The tested functions live in logos.role_auth; patching logos.main won't reach them.
_role_auth_impl = sys.modules["logos.role_auth"]
...
def patch_auth(monkeypatch):
    ...
    ctx.api_key_id = 1
    ctx.role = None
    # Patch the module where the functions are defined, not logos.main.
    monkeypatch.setattr(_role_auth_impl, "authenticate_api_key", lambda h: ctx)
...
def test_require_app_admin_or_above_passes_for_app_admin(monkeypatch):
    monkeypatch.setattr(_role_auth_impl, "_fetch_role", lambda key: "app_admin")
...
def test_require_app_admin_or_above_passes_for_logos_admin(monkeypatch):
    monkeypatch.setattr(_role_auth_impl, "_fetch_role", lambda key: "logos_admin")
    result = role_auth.require_app_admin_or_above(_make_request("key"))
    assert result == "key"


def test_require_app_admin_or_above_rejects_developer(monkeypatch):
    monkeypatch.setattr(_role_auth_impl, "_fetch_role", lambda key: "app_developer")
