...
def test_exact_canonical_name_wins():
    assert _resolve_requested_model_name("llama-3", [_row("llama"), _row("llama-3")]) == "llama-3"


def test_planner_alias_forms_resolve():
    assert _resolve_requested_model_name("llama", [_row("llama")]) == "llama"
    assert _resolve_requested_model_name("planner-llama", [_row("llama")]) == "llama"


def test_matching_is_case_insensitive():
    assert _resolve_requested_model_name("LLAMA", [_row("llama")]) == "llama"
    assert _resolve_requested_model_name("Planner-LLAMA", [_row("llama")]) == "llama"
    assert _resolve_requested_model_name("planner-LLAMA-2", [_row("llama")]) == "llama"


def test_sanitized_special_characters_resolve():
    assert _resolve_requested_model_name("org_model_tag", [_row("org/model:tag")]) == "org/model:tag"
    assert _resolve_requested_model_name("planner-org_model_tag", [_row("org/model:tag")]) == "org/model:tag"
...
def test_replica_suffix_resolves_to_model():
    ...
    assert _resolve_requested_model_name("planner-llama-2", [_row("llama")]) == "llama"
    assert _resolve_requested_model_name("planner-llama-7", [_row("llama")]) == "llama"
...
def test_replica_suffix_does_not_shadow_numbered_sibling():
    ...
    assert _resolve_requested_model_name("planner-llama-3", [_row("llama"), _row("llama-3")]) == "llama-3"
    assert _resolve_requested_model_name("planner-llama-3", [_row("llama-3"), _row("llama")]) == "llama-3"


def test_numbered_family_gemma():
    assert _resolve_requested_model_name("planner-gemma-2", [_row("gemma"), _row("gemma-2")]) == "gemma-2"
...
def test_replica_suffix_does_not_shadow_stored_alias():
    ...
    assert _resolve_requested_model_name("planner-llama-2", rows) is None


def test_genuine_replica_of_family_model_still_resolves():
    # The third replica of llama (index 4) is not any model's own alias.
    assert _resolve_requested_model_name("planner-llama-4", [_row("llama"), _row("llama-3")]) == "llama"


def test_replica_suffix_must_be_digit_at_least_two():
    assert _resolve_requested_model_name("planner-llama-1", [_row("llama")]) is None
    assert _resolve_requested_model_name("planner-llama-x", [_row("llama")]) is None
...
def test_replica_suffix_must_be_ascii_decimal():
    ...
    assert _resolve_requested_model_name("planner-llama-" + "٢", [_row("llama")]) is None
    assert _resolve_requested_model_name("planner-llama-" + "9" * 5000, [_row("llama")]) is None
    assert _resolve_requested_model_name("planner-llama-2", [_row("llama")]) == "llama"
...
def test_shared_planner_alias_is_ambiguous():
    ...
    assert _resolve_requested_model_name("planner-llama_2", [_row("llama/2"), _row("llama:2")]) is None
