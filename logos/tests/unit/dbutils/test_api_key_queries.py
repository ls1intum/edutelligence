...
def test_compute_cost_micro_cents_single_type():
    rows = [{"name": "prompt_tokens", "price_per_k_token": 200.0}]
    db = _db_fetchall(rows)
    result = db.compute_cost_micro_cents({"prompt_tokens": 1000, "completion_tokens": 0})
    assert result == 200


def test_compute_cost_micro_cents_multiple_types():
    rows = [
        {"name": "prompt_tokens", "price_per_k_token": 200.0},
        {"name": "completion_tokens", "price_per_k_token": 600.0},
    ]
    db = _db_fetchall(rows)
    result = db.compute_cost_micro_cents({"prompt_tokens": 500, "completion_tokens": 200})
    assert result == 220


def test_compute_cost_micro_cents_empty_usage():
    db = _db_fetchall([])
    result = db.compute_cost_micro_cents({})
    assert result == 0


def test_compute_cost_micro_cents_no_prices():
    db = _db_fetchall([])
    result = db.compute_cost_micro_cents({"prompt_tokens": 5000})
    assert result == 0


def test_compute_cost_micro_cents_unknown_type_skipped():
    rows = [{"name": "prompt_tokens", "price_per_k_token": 100.0}]
    db = _db_fetchall(rows)
    result = db.compute_cost_micro_cents({"prompt_tokens": 1000})
    assert result == 100


def test_record_budget_usage_zero_cost_skips():
    db = DBManager.__new__(DBManager)
    session = MagicMock()
    db.session = session

    db.record_budget_usage(api_key_id=7, month_start="2026-05-01", cost_micro_cents=0)
