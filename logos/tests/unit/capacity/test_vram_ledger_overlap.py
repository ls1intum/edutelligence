...
def test_disjoint_gpus_no_overlap():
    ledger = VRAMLedger()
    ledger.reserve(provider_id=1, lane_id="laneA", operation="load",
                   vram_mb=4096.0, gpu_devices="0")
...
def test_overlapping_gpus_detects_overlap():
    ledger = VRAMLedger()
    ledger.reserve(provider_id=1, lane_id="laneA", operation="load",
                   vram_mb=4096.0, gpu_devices="0,1")
...
def test_unspecified_target_gpus_treated_as_overlap():
    ...
    ledger = VRAMLedger()
    ledger.reserve(provider_id=1, lane_id="laneA", operation="load",
                   vram_mb=4096.0, gpu_devices=None)
