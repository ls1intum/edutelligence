...
3. Set API-Endpoints, Keys and Provider BaseURLs
...
class TestOpenAIForwardingProxy(unittest.TestCase):
    ...
    def test_connect_process_model(self):
        ...
        response = requests.post("http://logos.ase.cit.tum.de:8080/logosdb/connect_process_model", json=data, headers=headers)
        from pprint import pprint
        pprint(response.json())
        assert response.status_code == 200
    ...
    def test_connect_model_provider(self):
        ...
        response = requests.post("http://logos.ase.cit.tum.de:8080/logosdb/connect_model_provider", json=data, headers=headers)
        from pprint import pprint
        pprint(response.json())
        assert response.status_code == 200
    ...
    def test_connect_model_api(self):
        ...
        response = requests.post("http://logos.ase.cit.tum.de:8080/logosdb/connect_model_api", json=data, headers=headers)
        from pprint import pprint
        pprint(response.json())
        assert response.status_code == 200
    ...
    def test_forward_prompt(self):
        ...
        data = {
            "messages": [{"role": "user", "content": "Tell me a fun fact about the garamantian empire!"}],
            "temperature": 0.5
        }
