...
- **Inference gateway** (`gateway/`): one-query auth+permissions on the named-model cloud path; approximate monthly budget with a short TTL cache (see `GatewayBudgetService`); streaming cloud forward or orchestrator reverse-proxy. Remaining process state is named on `InferenceGatewayController`. A direct-cloud request is admitted against a flat reservation and then billed from the token usage its response reports (`GatewayUsageCapture` reads it off the stream, `GatewayCloudAccounting` stores it), so pricing runs through `logos_price_usage` exactly as on the orchestrator path — the reservation stands only when the response reports no usage, or when that usage prices to nothing (no price rows for the deployment). The row is written at the key's own `log` level, so a `FULL` key stores its request and response payloads; a streamed response has no single body to store.
