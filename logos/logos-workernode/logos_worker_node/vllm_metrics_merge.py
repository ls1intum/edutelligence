...
Used to fold several vLLM lanes' native ``/metrics`` text into one
deduplicated set of metric families, relabeling each source's samples so
identical metric names from different lanes don't collide into a single,
ambiguous series.
"""
...
logger = logging.getLogger("logos_worker_node.vllm_metrics_merge")
...
def merge_metric_families(sources: Iterable[tuple[dict[str, str], str]]) -> list[Metric]:
    ...
    metric name, with a distinguishing label (e.g. `lane_id`) per sample
