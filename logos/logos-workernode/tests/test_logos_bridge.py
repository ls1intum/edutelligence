...
@pytest.mark.asyncio
async def test_capability_announce_is_deferred_until_session_ends(tmp_path, monkeypatch):
    """A model calibrated mid-session must not reach capabilities_models
    before the session is over.

    Announcing early makes the server's capacity planner place a lane on this
    worker while the remaining models are still probing; the lane occupies the
    VRAM the probes need and every following kv-cache size fails with a bogus
    "free memory ... less than desired GPU memory utilization", which the tp
    escalation then reports as "model doesn't fit"."""
    from logos_worker_node import config as _wcfg
    from logos_worker_node.calibration import CalibrationResult
    ...
    cfg = LogosConfig(
        ...
        logos_url="https://logos.example",
        shared_key="secret",
        configured_models=["first/model", "second/model"],
    )
    client = LogosBridgeClient(app, cfg)

    # What capabilities_models looked like at the moment each model started
    # calibrating — the second entry is the one that matters.
    seen_capabilities: dict[str, list[str]] = {}

    def _fake_calibrate(plan, **kwargs):
        seen_capabilities[plan["model"]] = list(cfg.capabilities_models)
        return CalibrationResult(
            model=plan["model"],
            tensor_parallel_size=1,
            gpu_devices="0",
    ...
    # first/model finished before second/model started, yet must still have
    # been invisible to the server at that point.
    assert seen_capabilities["second/model"] == []
    # Once the session is over, both are announced.
    assert sorted(cfg.capabilities_models) == ["first/model", "second/model"]
    # capabilities_models is not part of the runtime dedupe signature, so the
    # publish must invalidate it or the announce can be suppressed.
    assert client._last_runtime_signature is None  # noqa: SLF001


@pytest.mark.asyncio
async def test_pending_capabilities_published_when_session_is_cancelled(tmp_path, monkeypatch):
    """A stop mid-session must still announce the models that did complete —
    their profiles are persisted, so holding them back would only delay
    serving them until the next worker restart."""
    from logos_worker_node import config as _wcfg
    from logos_worker_node.calibration import CalibrationResult
    ...
    def _fake_calibrate(plan, **kwargs):
        # Simulate a stop_calibration_session arriving while second/model is
        # being probed: first/model is already persisted and queued, the loop
        # then breaks on the cancel check before persisting second/model.
        if plan["model"] == "second/model":
            client._active_calibration_session.cancel_event.set()  # noqa: SLF001
        return CalibrationResult(
            model=plan["model"],
            tensor_parallel_size=1,
            gpu_devices="0",
    ...
    events = [(e.event, e.model) for e in app.state.lane_manager._event_log]
    assert ("calibration_session_cancelled", "") in events
    assert cfg.capabilities_models == ["first/model"]
