...
Options:
  --controller-url URL           LogosWorkerNode base URL (default: http://127.0.0.1:8444)
...
detect_logos_worker_node_container() {
  if ! has_cmd docker; then
    return 1
  fi

  local name
  name="$(docker ps --format '{{.Names}}' 2>/dev/null | awk '$0=="logos-workernode"{print $0; exit}')"
...
preflight_vllm_runtime() {
  ...
  if detect_logos_worker_node_container; then
    controller_hint="docker"
    if ! docker exec logos-workernode sh -lc 'command -v vllm >/dev/null 2>&1'; then
      cat >&2 <<'PREFLIGHT'
Detected containerized LogosWorkerNode ('logos-workernode') without a 'vllm' executable.
vLLM lanes cannot be created in this runtime.

Fix one of these before rerunning:
1) Run LogosWorkerNode on host .venv (recommended for vLLM lanes):
...
PREFLIGHT
    ...
    if ! docker exec logos-workernode sh -lc 'command -v nvcc >/dev/null 2>&1 || test -x /usr/local/cuda-12.8/bin/nvcc || test -x /usr/local/cuda/bin/nvcc'; then
      cat >&2 <<'PREFLIGHT'
Detected containerized LogosWorkerNode ('logos-workernode') without CUDA nvcc visibility.
...
2) Ensure docker-compose mounts CUDA toolkit into logos-workernode:
...
PREFLIGHT
  ...
  if [[ "$controller_hint" == "docker" ]]; then
    if ! docker exec logos-workernode sh -lc "test -x '$configured'"; then
      echo "Configured --vllm-binary '$configured' is not executable inside logos-workernode container." >&2
...
if ! curl -fsS "${CONTROLLER_URL%/}/health" >/dev/null; then
  echo "LogosWorkerNode health check failed at ${CONTROLLER_URL%/}/health" >&2
  exit 1
fi

preflight_vllm_runtime "$VLLM_BINARY"

echo "Clearing LogosWorkerNode lanes..."
clear_all_lanes
