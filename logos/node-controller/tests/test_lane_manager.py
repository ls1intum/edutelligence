...
from node_controller.lane_manager import LaneManager, PortAllocator
from node_controller.models import LaneConfig, OllamaConfig, ProcessState, ProcessStatus, VllmConfig


def test_port_allocator_skips_reserved_and_used_ports() -> None:
...
async def test_add_lane_releases_port_when_spawn_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr("node_controller.lane_manager._create_handle", _fake_create_handle)
...
async def test_build_lane_status_includes_vllm_runtime_fields() -> None:
    ...
    lane = LaneConfig(
        model="deepseek-ai/DeepSeek-R1-0528-Qwen3-8B",
        backend="vllm",
        vllm=VllmConfig(enable_sleep_mode=True, tensor_parallel_size=2),
    )
    ...
    assert status.backend_params["tensor_parallel_size"] == 2
    assert status.vram_reported_mb == pytest.approx(1.0)
    assert status.vram_by_pid_mb == pytest.approx(7680.0)
    assert status.vram_source == "pid"
    assert status.vram_used_mb == pytest.approx(7680.0)
...
async def test_sleep_lane_rejects_non_vllm_lane() -> None:
    ...
    lane = LaneConfig(model="qwen2.5-coder:32b", backend="ollama")
