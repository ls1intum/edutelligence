...
### Node Controller (config.yml)

The node controller supports this natively via the `llm_library` configuration field:
...
Changing `num_parallel` from 4 to 8 via the node controller took **~85 seconds**, with the model preload phase accounting for ~80s. The same model loaded in ~7s on the host Ollama service.
...
The node controller's `config.yml` supports these CUDA-related fields:
