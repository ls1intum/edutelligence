...
from node_controller.models import LaneConfig, OllamaConfig, ProcessState, ProcessStatus

logger = logging.getLogger("node_controller.vllm_process")
...
class VllmProcessHandle:
    ...
    async def is_sleeping(self) -> bool | None:
        ...
        if (
            ...
            or self._lane_config.backend != "vllm"
            or not self._lane_config.vllm.enable_sleep_mode
        ):
            return None
    ...
    def _ensure_sleep_mode_ready(self) -> None:
        ...
        if not lc.vllm.enable_sleep_mode:
            raise RuntimeError(
                f"[{self.lane_id}] Sleep mode is disabled for this lane. "
                "Enable lanes[].vllm.enable_sleep_mode=true and reconfigure."
            )
        if self._http is None:
    ...
    def _resolve_vllm_binary(self, configured_binary: str) -> str:
        ...
        raise FileNotFoundError(
            ...
            f"and '{venv_sibling}'. Set lanes[].vllm.vllm_binary to an absolute path "
