...
export default function Statistics() {
  ...
  const formatTimestampLabel = useCallback(
    (timestamp: string, period: string): string => {
  ...
  const modelBarData = useMemo(() => {
    return (stats?.modelBreakdown ?? []).slice(0, 6).map((m, index) => {
      const isCyan = index % 2 !== 0;
      return {
        value: m.requestCount || 0,
        label:
          m.modelName.length > 10 ? `${m.modelName.slice(0, 9)}…` : m.modelName,
                      ...
                      {(() => {
                        ...
                        if (isToday) {
                          ...
                          if (diffSec >= 0 && diffSec <= 86400) {
                            const bucketsFromStart =
                              diffSec / vramBucketSizeSec;
                            nowXPosition =
