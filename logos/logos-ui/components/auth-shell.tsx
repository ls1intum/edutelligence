...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    async (current: StoredTokens) => {
      ...
      try {
        const result = await AuthSession.refreshAsync(
          { clientId: KEYCLOAK_CLIENT_ID, refreshToken: current.refreshToken },
          oidcEndpoints
        );
        await persistTokens({
  ...
  }, [fetchRole, refreshTokens, clearSession]);
  ...
  const logout = useCallback(async () => {
    try {
      if (tokens?.refreshToken) {
        await fetch(oidcEndpoints.endSessionEndpoint, {
          method: "POST",
  ...
  const value = useMemo(
    () => ({ apiKey, status, role, userId, teams, roleError, reloadRole, completeLogin: persistTokens, logout }),
    [apiKey, status, role, userId, teams, roleError, reloadRole, persistTokens, logout]
  );
...
export function AuthenticatedShell({
  ...
  const { apiKey, status, role, roleError, reloadRole } = useAuth();
