...
export class AppKeysTabComponent {
  ...
  submitCreate(): void {
    ...
    this.svc.createApiKey(this.teamId, payload).subscribe({
      next: (newKey) => {
      ...
      error: (err) => {
        const msg = err?.error?.detail || err?.error?.message || 'Failed to create application key.';
  ...
  submitDelete(): void {
    ...
    this.svc.deleteApiKey(key.id).subscribe({
      next: () => {
  ...
  private loadGlobalData(pending?: TeamApiKey): void {
    ...
    forkJoin({
      ...
      teamProviders:  this.svc.getTeamProviderPermissions(this.teamId),
      teamModels:     this.svc.getTeamModelPermissions(this.teamId),
      ...
      await Promise.all(providers.map(async p => {
        try {
          const ms = await this.svc.getProviderModels(p.id).toPromise();
  ...
  private loadKeyPerms(keyId: number): void {
    ...
    forkJoin({
      providerIds: this.svc.getApiKeyProviderPermissions(keyId),
      modelIds:    this.svc.getApiKeyModelPermissions(keyId),
    }).subscribe({
