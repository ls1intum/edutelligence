...
export class RequestVolumeChartComponent implements OnChanges {
  ...
  readonly chartData = computed((): ChartData => {
    ...
    const empty: ChartData = {
      rects: [], totalPolyline: [], gridLines: [], xLabels: [],
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (mode === 'provider') {
        ...
        if (cVal > 0) stacks.push({ key: 'cloud', label: 'Cloud', color: CHART_ROLE.cloud, value: cVal });
        if (lVal > 0) stacks.push({ key: 'local', label: 'Local', color: CHART_ROLE.local, value: lVal });
      } else {
    ...
    for (let i = 0; i < buckets.length; i++) {
      ...
      for (const seg of b.stacks) {
        ...
        segRects.push({
          ...
          color: seg.color, isTop: false,
          label: seg.label, value: seg.value,
          timeLabel: b.timeLabel, seriesKey: seg.key,
        });
      }
