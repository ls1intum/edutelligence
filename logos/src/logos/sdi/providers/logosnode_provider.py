...
class LogosNodeDataProvider:
    ...
    def track_active_request(self, request_id: str, model_id: int, increment_active: bool) -> None:
        ...
        Unlike try_reserve_capacity (which gates admission), this is a
        no-gate write used by the request-tracking facade after admission
        has already been granted elsewhere.
        """
        with self._lock:
            if request_id in self._active_request_ids:
                return
