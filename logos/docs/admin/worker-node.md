...
5. Check that `http://localhost:80/` (or `http://localhost:$WORKER_PORT/` if
   you overrode it) returns the worker's service info, and check the
   provider status in the Logos UI.
