...
names, so read this first if the docs feel abstract. For diagrams of how the
pieces fit together (top-level design, deployment, data model, request flow),
see [System Design](./system-design).
...
The structural diagrams (top-level, deployment, data model, inference paths)
live on [System Design](./system-design). The gateway-split overview below is the
same story in one picture:

<img
  src={useBaseUrl('/img/architecture.svg')}
  alt="Logos architecture — core node, gateway split, worker nodes, and cloud providers"
  style={{width: '100%', height: 'auto'}}
/>

Editable source: <a href={useBaseUrl('/img/architecture.drawio')}>architecture.drawio</a>
